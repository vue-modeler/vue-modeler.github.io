...
::: warning
Внутри класса вызов `this.addProduct()` выглядит как обычный вызов метода, но на самом деле это не так. Декоратор `@action` подменяет оригинальный метод и "под капотом" получает действие как объект и вызывает метод `exec(...)`. Поэтому `try...catch` не будет работать, как ожидается. См. [Обработка ошибок](/ru/guides/action#обрабатываем-ошибки).
:::
