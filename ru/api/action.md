...
Экземпляр [`ActionError`](/ru/api/action-error), содержащий информацию об ошибке выполнения.

**Значение:**

- [`ActionError`](/ru/api/action-error) - если действие находится в состоянии `error`
- `null` - во всех остальных состояниях
...
6. Если метод возвращает Promise:
   ...
   - При ошибке: оборачивает ошибку в [`ActionError`](/ru/api/action-error) и устанавливает состояние в `error`
   - При прерывании: устанавливает состояние в `abort` или `lock` (если прервано блокировкой)

**Важно:** Ошибки выполнения оборачиваются в [`ActionError`](/ru/api/action-error) и сохраняются в состоянии `error`. Внешний `try/catch` не перехватит эти ошибки. Для обработки ошибок используйте проверку `action.error` после ожидания промиса или watcher по состоянию действия.
...
Интерфейс `ActionLike` (публичный контракт `Action`) вынесен в отдельный справочник: [Интерфейсы и типы](/ru/api/interfaces#actionlike).
