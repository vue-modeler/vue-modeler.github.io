...
**Действие** — это асинхронный метод класса модели, предназначенный для изменения состояния, который трансформируется в объект для контроля и управления выполнением операции.

<Aside type="note" title="Ключевая особенность:">
- **Объявляется как асинхронный метод** с декоратором `@action`
- **Используется как объект** с реактивными свойствами и собственным состоянием выполнения
</Aside>

**Характеристики действий:**

- **Асинхронность**. Действие работает с внешними источниками: сохраняет в БД, отправляет на сервер, поэтому объявляется как `async`
- **Не возвращает данных**. Результат выполнения операции - это новое состояние модели или ошибка, а не возвращаемые данные.
- **Трансформация** необходима для контроля и управления выполнением операции.
...
class CartModel extends ProtoModel {
    ...
    @action async addProduct(product): Promise<void> {
...
## Зачем трансформация в объект?

**Если коротко, что бы не писать шаблонный код.**

Разработчик пишет шаблонный код для каждой операции изменения состояния модели: 
...
4. Если сохранение:
    ...
    2. **не успешно** - устанавливает флаг ошибки и ошибка прокидывается дальше
...
Это тоже шаблонный код, но даже в одном проекте разработчики по разному обрабатывают эти случаи. 

Действие-объект позволяет разработчику сконцентрироваться на бизнес-логике (п. 2, 3, 4.1), остальное берет на себя, включая особые случаи.
