...
**Сигнатура:**
- `watch(source, callback, options?)` — наблюдает за источником
- `watchEffect(effect)` — наблюдает за эффектом
...
<Aside type="caution">
Не используйте `watch` или `watchEffect` напрямую из Vue Composition API внутри модели. Это приведет к утечкам памяти. Всегда используйте `this.watch()`.
</Aside>
